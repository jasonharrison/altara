...
class altara_socket(asynchat.async_chat):
    ...
    def found_terminator(self):
        ...
        # Do this after we get the server info! --- self.serverstore[SID] = {"servername": servername, "SID": SID, "serverdesc": serverdesc, "users": []}
            ...
            cuid = split[0].strip(":")
            uid = split[2]
            self.sendLine(":" + config.sid + " 311 " + cuid + " " + self.uidstore[uid]['nick'] + " " + self.uidstore[uid][
                'user'] + " " + self.uidstore[uid]['host'] + " * :" + self.uidstore[uid]['gecos'])
            self.sendLine(
                ":" + config.sid + " 312 " + cuid + " " + self.uidstore[uid]['nick'] + " " + config.servername.replace(
                    "(H) ", "") + " :" + config.serverdescription)
            ...
            if split[3] == "OPER":
                ...
                try:
                    ...
                    for modname, module in self.modules.items():
                        if hasattr(module, "onLogin"):
                            module.onLogin(self, uid, newaccount)
                except:
                    pass
        elif split[1] == "JOIN":
            uid = split[0].replace(":", "")
            ...
            if splitm[0].lower() == "modload" and self.uidstore[uid]['oper'] == True and target == self.reportchan:
                try:
                    modtoload = splitm[1]
                    self.sendNotice(self.mainc, config.reportchan,
                                    "Loading " + str(modtoload) + " (requested by " + nick + "!" + user + "@" + host + ")")
                    module = self.load("module_" + modtoload)
                    module.modinit(self)
                except Exception, e:
                    self.sendNotice(self.mainc, config.reportchan, "ERROR: " + (str(e)))
                # TODO: Reload/unload
            elif splitm[0].lower() == "modlist" and self.uidstore[uid]['oper'] == True and target == self.reportchan:
                # modname = splitm[1]
                # del self.modules[modname]
                self.sendNotice(self.mainc, config.reportchan, "Modules: " + str(self.modules.keys()))
            elif splitm[0].lower() == "modunload" and self.uidstore[uid]['oper'] == True and target == self.reportchan:
                try:
                    modname = splitm[1]
                    self.modunload(modname)
                    self.sendNotice(self.mainc, config.reportchan,
                                    "Unloaded " + modname + " (requested by " + nick + "!" + user + "@" + host + ")")
                except Exception, e:
                    self.sendNotice(self.mainc, config.reportchan, "ERROR: " + (str(e)))
            elif splitm[0].lower() == "modfullreload" and self.uidstore[uid]['oper'] == True and target == self.reportchan:
                try:
                    modname = splitm[1]
                    self.modules["module_" + modname].moddeinit(self)
                    ...
                    self.sendNotice(self.mainc, config.reportchan,
                                    "Reloaded " + modname + " (requested by " + nick + "!" + user + "@" + host + ")")
                except Exception, e:
                    self.sendLine("NOTICE " + uid + " :ERROR Reloading: " + (str(e) + " (is the module loaded?)"))
            # Reload without using modinit/deinit
            elif splitm[0].lower() == "modreload" and self.uidstore[uid]['oper'] == True and target == self.reportchan:
                try:
                    modname = splitm[1]
                    reload(self.modules["module_" + modname])
                    self.sendNotice(self.mainc, config.reportchan,
                    ...
                    self.sendLine("NOTICE " + uid + " :ERROR Reloading: " + (str(e) + " (is the module loaded?)"))
            elif splitm[0].lower() == "debugm" and self.uidstore[uid]['oper'] == True and target == self.reportchan:
